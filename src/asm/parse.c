...
/*   Updated: 2017/11/21 14:21:20 by jthillar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool		parse_second_read(t_header *header, t_instruction **list_instr, int fd)
{
	char			*line;
	t_instruction	*cursor;
	int				ret_gnl;
	...
	char *filename)
{
	...
	if ((fd = open(filename, O_RDONLY)) == -1)
		return (false);
	if (!(parse_second_read(header, list_instr, fd)))
		return (false);
	return (true);
}
